...
QueueHandle_t xQueueHydr8Buffer = NULL;     ///< Queue to send Hydr8 data to cloud
QueueHandle_t xQueueHydr8WLBuffer = NULL;		///< Queue to send Hydr8 Water Level data to cloud
QueueHandle_t xQueueHydr8TDSBuffer = NULL;		///< Queue to send Hydr8 TDS data to cloud
QueueHandle_t xQueueHydr8PumpLEDBuffer = NULL;  ///< Queue to send Hydr8 Pump LED status to cloud
QueueHandle_t xQueueHydr8TdsLEDBuffer = NULL;   ///< Queue to send Hydr8 TDS LED status to cloud
QueueHandle_t xQueueHydr8VolumeBuffer = NULL;   ///< Queue to send volume data to cloud
...
// static void MQTT_HandleHydr8Messages(void)
// {
// 	struct Hydr8DataPacket hydr8Data;
// 	if (pdPASS == xQueueReceive(xQueueHydr8Buffer, &hydr8Data, 0)) {
// 		snprintf(mqtt_msg, 63, "{\"wl\":%d, \"tds\":%d}", hydr8Data.waterLevel,hydr8Data.tdsLevel);
...
/**
 void WifiAddHydr8DataToQueue(struct Hydr8DataPacket* hydr8Packet)
 * @brief	Adds an IMU struct to the queue to send via MQTT
 * @param[out]
...
// {
//     int error = xQueueSend(xQueueHydr8Buffer, hydr8Packet, (TickType_t)10);
//     return error;
// }

/**
int WifiAddHydr8WLDataToQueue(struct Hydr8WaterLevel *hydr8WaterData)
 * @brief	Adds a Water level struct element to the queue to send via MQTT
 * @param[out]
...
/**
int WifiAddHydr8TDSDataToQueue(struct Hydr8WaterLevel *hydr8WaterData)
 * @brief	Adds a Water level struct element to the queue to send via MQTT
 * @param[out]
 ...
 * @brief	Adds a pump led struct element to the queue to send via MQTT
 * @param[out]
...
/**
int WifiAddHydr8VolumeToQueue(struct pumpLed *pumpLedStatus)
 * @brief	Adds a volume element to the queue to send via MQTT
 * @param[out]
 ...
 * @brief	Adds a tds led struct element to the queue to send via MQTT
 * @param[out]
